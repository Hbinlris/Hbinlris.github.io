<template>
  <el-row>
    <el-col :span="5">
      ...
      <el-statistic title="用户数量" :value="userCountTransition" />
    </el-col>
    <el-col :span="5">
      ...
      <el-statistic :value="pendingReviewCountTransition">
        <template #title>
        ...
        <template #suffix>/{{ Math.floor(noteCountTransition) }}</template>
      </el-statistic>
    </el-col>
    <el-col :span="5">
      ...
      <el-statistic title="评论数量" :value="commentCountTransition" />
    </el-col>
    <el-col :span="5">
      ...
      <el-statistic title="笔记数量" :value="noteCountTransition">
  ...
  <div style="display: flex">
    <div class="dashboard" style="width: 717px;">
      <div
        ref="barChart"
        style="
          width: 600px;
    ...
    <div class="dashboard" style="width: 310px">
    ...
    <div class="dashboard" style="width: 325px">
      <div
        ref="roseChart"
        style="
          width: 285px;
        ...
        "
      ></div>
      <div style="font-size: 20px; font-weight: bold">标签使用占比</div>
    </div>
  </div>
</template>
...
export default {
  ...
  setup() {
    ...
    const noteCountTransition = useTransition(noteCount, {
      duration: 1500,
    });
    const commentCount = ref(0); // 用于存储评论数量
    const commentCountTransition = useTransition(commentCount, {
      duration: 1500,
    });

    const userCount = ref(0); // 用于存储用户数量
    const userCountTransition = useTransition(userCount, {
      duration: 1500,
    });

    const pendingReviewCount = ref(0); // 用于存储待审核的数量
    const pendingReviewCountTransition = useTransition(pendingReviewCount, {
      duration: 1500,
    });
    ...
    const initBarChart = () => {
      ...
      const seriesData1 = honeyData.value.map((item) =>
        parseInt(item.tags, 10)
      );
      const seriesData2 = honeyData.value.map(
        (item) => parseInt(item.tags, 10) * 2
      ); // 示例数据，可以根据需要调整
    ...
    return {
      ...
      outputValue,
      noteCountTransition,
      commentCountTransition,
      userCountTransition,
      pendingReviewCountTransition,
      noteCount,
      commentCount,
      userCount,
      pendingReviewCount,
