  ...
  <div class="note">
    <div class="notebox" v-if="datalist.length != 0">
      <ul style="
          ...
          margin-top: 10rem;
        ">
        <li v-for="item in datalist" :key="item.id" style="width: 160rem; margin-bottom: 10rem" @click="
          $router.push({
            path: '/NoteDetailPage',
            query: { id: item.id },
          })
          ">
          <div class="up">
            <van-image width="160rem" radius="10rem" fit="cover" :src="item.entity.img" />
          </div>
          <!-- {{ item.entity.img }} -->
          <div class="down">
            ...
            <div class="msg" style="display: flex; align-items: center; margin-top: 5rem">
              <!-- 用户头像 -->
              <van-image width="10rem" radius="50%" :src="item.author.p"></van-image>
              <span style="color: #a7a7a7; font-size: 9rem; margin-left: 3rem">{{ item.author.n }}</span>
            </div>
          </div>
        </li>
      </ul>
